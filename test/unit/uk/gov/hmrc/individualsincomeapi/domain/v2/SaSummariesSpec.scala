...
import uk.gov.hmrc.individualsincomeapi.domain.v2.SaSummaries
import utils.IncomeSaHelpers
import org.scalatest.matchers.should.Matchers
import org.scalatest.wordspec.AnyWordSpec

class SaSummariesSpec extends AnyWordSpec with Matchers with IncomeSaHelpers {
  ...
  "SaSummaries" should {
    "Write to Json for verbose data" in {
      ...
      val result = Json.toJson(SaSummaries.transform(ifSa))
    ...
    "Write to Json for no root data" in {
      ...
      val result = Json.toJson(SaSummaries.transform(Seq()))
    ...
    "Write to Json with defaults no data containers" in {
      ...
      val result = Json.toJson(SaSummaries.transform(ifSaNoData))
    ...
    "Write to Json with defaults no vales" in {
      ...
      val result = Json.toJson(SaSummaries.transform(ifSaNoValues))
