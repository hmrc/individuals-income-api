...
import uk.gov.hmrc.individualsincomeapi.controllers.IncomeController
...
import uk.gov.hmrc.individualsincomeapi.services.IncomeService
...
class IncomeControllerSpec extends UnitSpec with MockitoSugar  with WithFakeApplication {
  ...
  trait Setup {
    ...
    val testIncomeController = new IncomeController(mockIncomeService) {}
  }
  ...
  "income controller income function" should {
    ...
    "return 200 (OK) when matching succeeds and service returns payments" in new Setup {
      ...
      val result = await(testIncomeController.income(matchId.toString, interval)(fakeRequest))
    ...
    "return 404 (Not Found) for a malformed matchId" in new Setup {
      val result = await(testIncomeController.income("malformedMatchId", interval)(fakeRequest))
